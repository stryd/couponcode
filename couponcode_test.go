...
func TestCodesNormalized(t *testing.T) {
	var runs = []struct {
		...
		{Default, "i9oD/V467", "190D-V467"},   // alternate separator
		{Default, " i9oD V467 ", "190D-V467"}, // whitespace accepted
		{Default, " i9oD_V467 ", "190D-V467"}, // underscores accepted
		{Default, "i9oDV467", "190D-V467"},    // no separator required
	}
	for _, run := range runs {
		validated, err := run.g.Validate(run.code)
...
func TestPattern(t *testing.T) {
	...
	matched, _ = regexp.MatchString(`^\w{4}-\w{4}$`, code)
	if !matched {
		t.Error("should look like XXXX-XXXX")
	}
	g := New(2, 5)
	code = g.Generate()
